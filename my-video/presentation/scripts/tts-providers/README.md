...
| `qwen.sh` | Qwen3-TTS (moma.hq.cmcc) | JWT Token (Authorization header) | 内部部署；中文音质优秀 |
...
## Qwen3-TTS Provider 详细说明

### 快速开始

```bash
# 1. 配置环境变量
export QWEN_TTS_API_KEY="eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9..."
...
**响应格式**：
- 成功：直接返回 MP3 音频二进制流（Content-Type: audio/mpeg）
- 失败：返回 JSON 错误
