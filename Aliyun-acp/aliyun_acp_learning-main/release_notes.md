...
开发者已经可以用 Qwen Code 完成真实任务，也可以用 Skill 固化经验、用评测检查结果。但在业务工作流里，新的问题很快会出现：一次任务跑通了，换主题、换要求、换上下文后还能不能稳定复用？本次更新新增 **3.8 用 Harness Engineering 和 Loop Engineering 打造可上线的业务能力**，通过技术文档流程图实验，带学员把标准、工具、截图验证、运行记录、Reviewer 角色、停止条件和反馈机制组织成一条可持续改进的工作流。
